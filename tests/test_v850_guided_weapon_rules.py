...
class GuidedWeaponRuleTests(unittest.TestCase):
    def test_01_version_850(self):
        self.assertIn("const APP_VERSION = '8.6.12';",APP); self.assertIn("const APP_VERSION = '8.6.12';",WORKER); self.assertIn('V8.6.12',INDEX)
    ...
    def test_32_release_notes_and_accessibility(self):
        self.assertIn('## v8.6.12',README); self.assertIn('aria-live="polite"',APP); self.assertIn('data-weapon-rule-target',APP); self.assertIn('role="status"',APP)
