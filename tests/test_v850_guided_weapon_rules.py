...
class GuidedWeaponRuleTests(unittest.TestCase):
    def test_01_version_850(self):
        self.assertIn("const APP_VERSION = '8.5.0';",APP); self.assertIn("const APP_VERSION = '8.5.0';",WORKER); self.assertIn('V8.5.0',INDEX)
    ...
    def test_12_13_seek_light(self):
        self.assertIn('Is the target visible and using Light terrain for cover?',APP); self.assertIn('It does not remove a cover save.',APP); self.assertIn("target?.order!=='Conceal'",APP); self.assertIn('seekLightAnswer',APP)
    def test_14_to_16_blast(self):
        self.assertIn('including friendly operatives',APP); self.assertIn("ruleId==='blast'?[...playerTargets,...npoTargets]",APP); self.assertIn("secondaryTargetIds:targets.slice(1)",APP)
    ...
    def test_32_release_notes_and_accessibility(self):
        self.assertIn('## v8.5.0',README); self.assertIn('aria-live="polite"',APP); self.assertIn('data-weapon-rule-target',APP); self.assertIn('role="status"',APP)
