...
(async()=>{
  ...
  const engine=api.createMissionEngine({requestDiceRoll:async()=>rolls.shift(),setOperativeInPlay:async operation=>{battlefield.set(operation.operativeId,operation);return true;}});
  ...
  await engine.executeMissionAction('recordEscape',{...context,operativeId:'alpha',gameplay:{...context.gameplay,escapedOperativeCount:1}});
  await engine.executeMissionAction('recordEscape',{...context,turningPoint:2,operativeId:'bravo',gameplay:{...context.gameplay,escapedOperativeCount:2}});
  ...
  const completed=await engine.executeMissionAction('recordEscape',{...context,turningPoint:2,operativeId:'charlie',gameplay:{...context.gameplay,escapedOperativeCount:3}});
  assert.equal(engine.getObjectiveValue('escapedOperatives'),3);assert.equal(battlefield.get('charlie').reason,'escaped');assert.equal(engine.getMissionHudModel().completed,true);
  assert.deepEqual(engine.getMissionRuntime().history.at(-1).completedObjectiveIds,['escapedOperatives']);
  const afterCompletion=await engine.executeMissionAction('recordEscape',{...context,turningPoint:2,operativeId:'delta',gameplay:{...context.gameplay,escapedOperativeCount:4}});
  assert.equal(engine.getObjectiveValue('escapedOperatives'),4);assert.deepEqual(engine.getMissionRuntime().history.at(-1).completedObjectiveIds,[]);
  assert.equal(definition.completion.endsBattle,false);
  await engine.executeMissionAction('correctEscape',{...context,turningPoint:2,operativeId:'delta',gameplay:{...context.gameplay,escapedOperativeCount:3}});
  assert.equal(engine.getMissionHudModel().completed,true);
  await engine.executeMissionAction('correctEscape',{...context,turningPoint:2,operativeId:'charlie',gameplay:{...context.gameplay,escapedOperativeCount:2}});
  assert.equal(engine.getMissionHudModel().completed,false);
  await engine.executeMissionAction('recordEscape',{...context,turningPoint:2,operativeId:'charlie',gameplay:{...context.gameplay,escapedOperativeCount:3}});
...
(async()=>{
  ...
  const floor=api.createMissionEngine({setOperativeInPlay:async()=>true});floor.initializeMissionRuntime(copy(),{gameplay:{playerOperativeCount:4}});
  await floor.executeMissionAction('correctEscape',{operativeId:'alpha',gameplay:{escapedOperativeCount:0}});assert.equal(floor.getObjectiveValue('escapedOperatives'),0);
})().catch(error=>{console.error(error);process.exit(1)});
""")
