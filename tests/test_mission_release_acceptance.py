...
"""
    ...
    def test_release_assets_versions_and_mission_scope_are_consistent(self):
        ...
        self.assertIn("const APP_VERSION = '6.0.0'", app)
        self.assertIn("const APP_VERSION = '6.0.0'", worker)
        self.assertIn("V6.0.0", index)
        for asset in ("app.js", "mission-engine.js", "persistence.js", "styles.css"):
            self.assertIn(f"{asset}?v=6.0.0", index)
        for asset in (
        ...
        self.assertEqual(
            len(re.findall(r"definition-[^'\"]+\.json", worker)), 1
        )
