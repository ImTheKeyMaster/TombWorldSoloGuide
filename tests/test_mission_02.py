...
""")
    ...
    def test_registry_features_ui_save_refresh_and_reference_mission_regressions(self):
        ...
        self.assertIn("if(missionEngine(selectedMission)?.type==='sabotage')objectiveEngine.setObjectiveValue('sabotagedFeatures',state.missionState?.completedFeatureIds?.length||0", app)
        self.assertNotIn("if(!restoringRuntime&&missionEngine(selectedMission)?.type==='sabotage')", app)
        self.assertIn("$$('[data-mission-feature]').forEach(control=>{control.disabled=true;})", app)
        self.assertIn("$$('[data-mission-feature]').forEach(control=>{control.disabled=false;})", app)
