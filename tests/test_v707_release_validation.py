...
const state={{version:'8.6.18',saveVersion:persistence.currentSaveVersion(),missionId:'01',
...
(async()=>{
  ...
  assert.equal(added[0].name,'tomb-world-solo-guide-8.6.18');
  assert.ok(added[0].assets.includes('./index.html'));
  assert.ok(added[0].assets.includes('./app.js?v=8.6.18'));
