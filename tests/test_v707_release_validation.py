...
const state={{version:'7.4.0',saveVersion:persistence.currentSaveVersion(),missionId:'01',
...
(async()=>{
  ...
  assert.equal(added[0].name,'tomb-world-solo-guide-7.4.0');
  assert.ok(added[0].assets.includes('./index.html'));
  assert.ok(added[0].assets.includes('./app.js?v=7.4.0'));
...
"""
    ...
    def test_release_version_help_and_accessible_elimination_text_are_current(self):
        self.assertIn("const APP_VERSION = '7.4.0';", APP)
        self.assertIn("const APP_VERSION = '7.4.0';", WORKER)
        self.assertIn("V7.4.0", INDEX)
        for asset in ("app.js", "mission-engine.js", "persistence.js", "styles.css"):
            self.assertIn(f"{asset}?v=7.4.0", INDEX)
