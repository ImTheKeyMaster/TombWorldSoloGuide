...
(async()=>{
  ...
  assert.equal(added[0].name,'tomb-world-solo-guide-8.6.35');
  assert.ok(added[0].assets.includes('./index.html'));
  assert.ok(added[0].assets.includes('./app.js?v=8.6.35'));
...
"""
    ...
    def test_release_version_help_and_accessible_elimination_text_are_current(self):
        ...
        self.assertIn("const APP_VERSION = '8.6.35';", WORKER)
        self.assertIn("V8.6.35", INDEX)
        for asset in ("app.js", "mission-engine.js", "persistence.js", "styles.css"):
            self.assertIn(f"{asset}?v=8.6.35", INDEX)
