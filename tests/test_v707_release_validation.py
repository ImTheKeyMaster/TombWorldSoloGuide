...
const state={{version:'7.5.8',saveVersion:persistence.currentSaveVersion(),missionId:'01',
...
(async()=>{
  ...
  assert.equal(added[0].name,'tomb-world-solo-guide-7.5.8');
  assert.ok(added[0].assets.includes('./index.html'));
  assert.ok(added[0].assets.includes('./app.js?v=7.5.8'));
