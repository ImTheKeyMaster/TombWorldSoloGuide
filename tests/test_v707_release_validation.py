...
const state={{version:'8.6.14',saveVersion:persistence.currentSaveVersion(),missionId:'01',
...
(async()=>{
  ...
  assert.equal(added[0].name,'tomb-world-solo-guide-8.6.14');
  assert.ok(added[0].assets.includes('./index.html'));
  assert.ok(added[0].assets.includes('./app.js?v=8.6.14'));
...
"""
    ...
    def test_release_version_help_and_accessible_elimination_text_are_current(self):
        ...
        self.assertIn("const APP_VERSION = '8.6.14';", WORKER)
