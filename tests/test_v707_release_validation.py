...
const state={{version:'7.6.3',saveVersion:persistence.currentSaveVersion(),missionId:'01',
...
(async()=>{
  ...
  assert.equal(added[0].name,'tomb-world-solo-guide-7.6.3');
  assert.ok(added[0].assets.includes('./index.html'));
  assert.ok(added[0].assets.includes('./app.js?v=7.6.3'));
...
"""
    ...
    def test_release_version_help_and_accessible_elimination_text_are_current(self):
        self.assertIn("const APP_VERSION = '7.6.3';", APP)
        self.assertIn("const APP_VERSION = '7.6.3';", WORKER)
        self.assertIn("V7.6.3", INDEX)
        for asset in ("app.js", "mission-engine.js", "persistence.js", "styles.css"):
            self.assertIn(f"{asset}?v=7.6.3", INDEX)
