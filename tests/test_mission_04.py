...
(async()=>{
  ...
  const breach=await engine.executeMissionAction('breachSarcophagus',{turningPoint:1,activationId:'1:a'});
  ...
  await engine.executeMissionAction('breachSarcophagus',{turningPoint:3,activationId:`3:${rolls.length}`});
  const completion=await engine.executeMissionAction('breachSarcophagus',{turningPoint:3,activationId:`3:${rolls.length}`});
  assert.equal(engine.getObjectiveValue('destructionPoints'),20);assert.equal(completion.changes[0].after,20);
  assert.equal(runtime.objectives.destructionPoints.completed,true);assert.equal(runtime.objectives.destructionPoints.completedTurningPoint,3);assert.equal(definition.completion.endsBattle,true);assert.equal(victory,false);
...
"""
    ...
    def test_mission_ui_and_lifecycle_integration_remain_generic(self):
