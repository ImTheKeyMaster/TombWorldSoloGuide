(() => {
  ...
  function rollInitiative(){
    ...
    if(state.threat===0){state.strategyData.playerRoll=null;state.strategyData.npoRoll=null;state.strategyData.suggestedInitiative='player';return;}
    const e=roll(),n=roll();state.strategyData.playerRoll=e;state.strategyData.npoRoll=n;
    state.strategyData.suggestedInitiative=n>e?'npo':'player';
  }
  ...
  const npoQuestions = [
    {key:'engaged',title:'Is an Player operative in control range?',help:'Choose Yes when this NPO can immediately Fight an Player operative without moving.'},
    {key:'charge',title:'Can this NPO complete a Charge?',help:'Choose Yes only when a legal Charge can finish within control range of an Player operative.'},
    ...
    {key:'objective',title:'Is an Player operative controlling a mission objective?',help:'This gives mission denial priority over an otherwise equal target.'},
    ...
    {key:'hatch',title:'Does a closed hatch block the best route?',help:'Choose Yes when operating the hatch is the clearest way to advance toward an Player or objective.'},
  ...
  function chooseNpoDecision(n,c){
    ...
    if(c.engaged){
      ...
      reason='An Player is already in control range, so immediate melee takes priority.';threat=1;path.unshift('Player in control range → Fight');
  ...
  function showNpoAttackWizard(n,attackDice,onDone){
    showModal('NPO Attack Wizard',`<p>${escapeHtml(n.name)} rolled the attack dice shown below. Enter the Player operative’s defense profile to resolve saves and damage.</p>
      ...
      <div class="combat-grid">
        ...
        ${spinnerField('playerSave','Player Save',3,2,6)}
        ${spinnerField('npoAp','NPO AP',0,0,3)}
        ${spinnerField('playerWounds','Player wounds remaining',10,1,30)}
      </div>
    ...
    $('#rollNpoSaves').onclick=()=>{
      ...
      $('#rollNpoSaves').onclick=()=>{log(`${n.name} dealt ${result.damage} damage to an Player operative (${before} → ${after} wounds).`);save();closeModal();if(onDone)onDone();};
