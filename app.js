...
>
  ...
  let dashboardController=null, stopDashboardCamera=null, dashboardCountdown=null;
  ...
  async function openDashboardPairing(reestablish=false){
    ...
    try{
      ...
      controller.subscribeDashboardStatus(detail=>{const node=$('#dashboardPairingStatus');if(node)node.textContent=detail.text;});
  ...
  async function addDashboardGameMenuSection(){
    ...
    try{const feature=await requestDashboardFeature();const online=feature.isDashboardWebRtcSupported()&&await feature.requestDashboardAvailability({gameMenuOpen:true,activeGame:true,isEligible:()=>Boolean($('#dashboardMenuSection')&&state.screen==='game')});if(!host.isConnected)return;const controller=await getDashboardController();const detail=controller.getDashboardStatus();host.innerHTML=`<h3>Dashboard</h3><p class="dashboard-menu-status" role="status" aria-live="polite">${detail.text}</p>${online?`<button class="btn secondary" id="menuSetupDashboard">${detail.hasAttempt?'Reestablish Dashboard Pairing':'Setup Dashboard'}</button>`:''}`;controller.subscribeDashboardStatus(next=>{const node=$('.dashboard-menu-status',host);if(node)node.textContent=next.text;});if($('#menuSetupDashboard'))$('#menuSetupDashboard').onclick=()=>openDashboardPairing(detail.hasAttempt);}catch{host.innerHTML='<h3>Dashboard</h3><p class="dashboard-menu-status">Not connected</p>';}
  }
