...
>
  ...
  async function openDashboardPairing(reestablish=false){
    ...
    try{
      ...
      scanButton.onclick=async()=>{if(stopDashboardCamera){stopPairingCamera();cameraHost.replaceChildren();resetScanControls();return;}stopPairingCamera();showScanner();controller.markWaitingForResponse();scanButton.textContent='Cancel Scan';try{await scanQrCode({videoHost:$('#dashboardVideoHost'),onResult:apply,onStop:resetScanControls,onCleanupReady:cleanup=>{stopDashboardCamera=cleanup;}});}catch(error){cameraError(error);}};
