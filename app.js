...
document.addEventListener('touchend',function(e){
  ...
  function deadlyEncountersStatusLabel(){return deadlyEncountersActive()?'On':(isPvpMode()?'Unavailable (Solo only)':'Off');}
...
>
  ...
  function renderMission(){
    ...
    app.innerHTML=`<div class="panel-title"><div><p class="eyebrow">MISSION</p><h2>${m.number} · ${m.name}</h2><p>${escapeHtml(presentSideTerminology(m.brief))}</p></div></div>
      ...
      ${boardSvg(m.id)}
      <section class="card"><h3>Battle settings</h3><p><strong>Restless Tomb:</strong> ${state.restlessTombEnabled?'On':'Off'} (House Rule)</p><p><strong>Deadly Encounters:</strong> ${deadlyEncountersStatusLabel()} (Official Expansion - White Dwarf 521)</p></section>
  ...
  function showGameMenu(){
    ...
    showModal('Game Menu',`<p>Open a reference screen without changing the guided play sequence, or begin a completely new game.</p>
      <div class="game-menu-grid">
        ...
        ${!isPvpMode()?'<button class="btn secondary" id="menuDeadlyEncounters">Deadly Encounters</button>':''}
    ...
    if(inGame){
      ...
      if(!isPvpMode())$('#menuDeadlyEncounters').onclick=showDeadlyEncountersPanel;
    }
