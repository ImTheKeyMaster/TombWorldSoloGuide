...
document.addEventListener('touchend',function(e){
  ...
  const tombsBeyondCountingEventDefinitions=Object.freeze({
    ...
    'rewards-of-annihilation':{title:'Rewards of Annihilation',text:'Until the end of the turning point, whenever a Skorpekh Destroyer or Hexmark Destroyer incapacitates a Player operative, the Guide automatically rolls D3 and restores that many lost wounds to that NPO, or rolls 2D3 if that operative has a Wounds characteristic of 12 or more.',resultText:'Effect active until the end of this Turning Point.',execution:{type:'activate'},lifecycle:'persistent',duration:'turning-point',handlerId:'rewards-of-annihilation',gameplayHooks:[],automationType:'automatic',priority:20},
  ...
  const npoDefinitions = {
    ...
    'Geomancer': {
      ...
      actions:[
        {id:'geomantic-disturbance',name:'Geomantic Disturbance',ap:1,target:{kind:'terrain-point',visible:true,range:8,affectedOperativesWithin:2},restrictions:{ordersExcluded:['Conceal'],actorOutsideEnemyControlRange:true},resolution:{manualTabletopSelection:true,dicePerTarget:'2D6',separateRollPerTarget:true,damage:'roll-minus-remaining-wounds-if-positive'},description:'1. Choose a terrain point the Geomancer can see within 8 inches. 2. Select every operative within 2 inches of that point. 3. The Guide automatically rolls 2D6 separately for each selected operative and resolves the damage. If the total is higher than its remaining wounds, deal damage equal to the difference. Cannot be used while the Geomancer has a Conceal order or is in enemy control range.'},
    ...
    'Canoptek Macrocyte Warrior': {
      ...
      ],abilities:[{id:'aggressive-defence',name:'Aggressive Defence',text:'When an enemy operative within 2 inches incapacitates this operative, the Guide automatically rolls D3 before removal; on 2+, inflict exactly 1 damage on that enemy.'},{id:'expendable-construct',name:'Expendable Construct',text:'Ignore this operative for scoring conditions that require operatives to escape, survive, or be incapacitated.'}],
    ...
    'Canoptek Macrocyte Reanimator': {
      ...
      actions:[{id:'nanoscarab-beam',name:'Nanoscarab Beam',ap:1,oncePerTurningPoint:true,target:{side:'friendly',keywordsAll:['Canoptek Circle'],visible:true,range:6,notIncapacitated:true,excludeReanimatedThisTurningPoint:true},restrictions:{actorOutsideEnemyControlRange:true},healing:{dice:'3D3',capAtMaximum:true,recordRolledAndRestored:true},description:'Choose a visible wounded Canoptek Circle NPO within 6 inches. The Guide automatically rolls 3D3 and restores that many wounds, up to its maximum. It cannot target an incapacitated NPO or one saved by Reanimate this turning point.'}],
  ...
  const eventDefinitions = {
    'subjugation-glyphs':{title:'Subjugation Glyphs',text:'The Guide automatically selects and tests eligible Player operatives without replacement, rolling D6 for each. If a result is higher than an operative’s effective APL, subtract 1 from its APL.',execution:{type:'subjugation-glyphs'},lifecycle:'immediate',duration:'persistent',handlerId:'subjugation-glyphs',gameplayHooks:['effectivePlayerApl'],automationType:'automatic',priority:10},
    'transdimensional-relocation':{title:'Transdimensional Relocation',text:'The Guide automatically selects two random Player operatives. Swap their positions.',execution:{type:'transdimensional-relocation'},duration:'immediate',redrawIfImpossible:true},
    ...
    'countertemporal-shifting':{title:'Countertemporal Shifting',text:'Until the end of the turning point, when an attack die would inflict 3 or more damage on an NPO, the Guide automatically rolls one D6. On 5+, subtract 1 damage from that die.',execution:{type:'activate'},lifecycle:'persistent',duration:'turning-point',handlerId:'countertemporal-shifting',gameplayHooks:['damagePackets'],automationType:'automatic',priority:30},
    'living-metal-flux':{title:'Living Metal Flux',text:'For each NPO that has lost any wounds, the Guide automatically rolls D3 and restores that result plus 2 wounds.',execution:{type:'living-metal-flux'},duration:'immediate'},
    'maze-reforms':{title:'The Maze Reforms',text:'The Guide automatically rolls D3 to determine the maximum number of open hatchways you can close. Close one breach and up to that many open hatchways. If this cannot be resolved, draw another event card.',execution:{type:'maze-reforms'},duration:'immediate',redrawIfImpossible:true},
  ...
  function normalizeState(raw){
    ...
    const missionType=missionEngine(savedMission)?.type;
    const livingSavedPlayers=merged.playerRoster.filter(id=>!merged.playerCasualtyIds.includes(id)).length;
    const objectiveWasAchieved=missionType==='escape'
      ? new Set([...(merged.missionState?.escapedIds||[]),...merged.playerCasualtyIds]).size>=merged.playerRoster.length
      : missionType==='sabotage'?(merged.missionState?.completedFeatureIds||[]).length>=missionEngine(savedMission).required
      : missionType==='transponder'?Boolean(merged.missionState?.escaped)
      : missionType==='destruction'?Number(merged.missionState?.destruction||0)>=missionEngine(savedMission).required
      : missionType==='scout'?(merged.missionState?.scoutedRoomIds||[]).length>=missionEngine(savedMission).required
      : missionType==='regroup'?merged.playerRoster.filter(id=>!merged.playerCasualtyIds.includes(id)&&merged.playerOperativeStates[id]?.inPlay!==false).every(id=>{
          const check=merged.missionState?.operativeChecks?.[id]||{};
          return check.inDropZone&&check.outsideNpoControl&&check.nearPlayer;
        })
      : false;
    const obsoleteLimitDefeat=merged.turningPoint===4&&merged.gameEnd==='defeat'
      &&merged.finalResolution.turningPointEnded&&merged.finalResolution.cleanupComplete
      &&livingSavedPlayers>0&&!objectiveWasAchieved;
    if(obsoleteLimitDefeat){
      merged.gameEnd=null;merged.completed=false;merged.phase='between';
      merged.finalResolution={...merged.finalResolution,pending:false,turningPointEnded:true,cleanupComplete:true,battleEndHookComplete:false,resultLogged:false,invalidSaveCorrected:true};
      const correction='Battle restored because Tomb World Joint Ops does not automatically end after Turning Point 4.';
      if(!merged.journal.some(entry=>entry.text===correction))merged.journal.unshift({time:new Date().toISOString(),text:correction});
    }
  ...
  function shiftingLabyrinthResult(progress=state.missionState||freshMissionState()){
    const total=state.playerRoster.length;
    const escaped=new Set((progress.escapedIds||[]).filter(id=>state.playerRoster.includes(id)));
    const removed=new Set([...escaped,...(state.playerCasualtyIds||[]).filter(id=>state.playerRoster.includes(id))]);
    const requiredEscapes=Math.ceil(total/2);
    if(!total||removed.size<total)return {outcome:null,total,escaped:escaped.size,requiredEscapes};
  ...
  function missionOutcomeExplanation(outcome){
    ...
    const threshold=result.escaped>=result.requiredEscapes;
    return `${result.escaped} of ${result.total} operatives escaped. ${threshold?'At least half of':'Fewer than half of'} the player operatives escaped after every operative was removed from the killzone.`;
  }

  const missionOutcomeEvaluators = {
    escape:(engine,progress)=>shiftingLabyrinthResult(progress).outcome,
  ...
  function guideInstructionsHtml(full=false){
    ...
    const flow=`<section class="help-section">
      ...
      <ol class="guide-flow-list">
        <li><strong>Set up the mission</strong><span>Choose a mission and build the killzone. The Guide automatically generates the starting NPO roster; you deploy both sides.</span></li>
        <li><strong>Prepare the Turning Point</strong><span>The Guide readies operatives, applies mission Ready rules, automatically rolls one D6 for each side to determine initiative after Turning Point 1, then automatically determines and generates reinforcements. You place generated models on the tabletop.</span></li>
  ...
  function renderGame(){
    if(state.gameEnd){
      ...
      app.innerHTML=`<section class="hero-card mission-outcome"><p class="eyebrow">BATTLE COMPLETE</p><img class="game-end-image" src="Assets/Images/${resultClass}.png" alt=""><h2>Battle Complete</h2><h3 class="battle-result battle-result--${resultClass}">MISSION ${resultLabel.toUpperCase()}</h3><p>${escapeHtml(missionOutcomeExplanation(state.gameEnd))}</p>${missionProgressHtml(true)}<div class="button-row"><button class="btn secondary" id="reviewCompletedMission">Review Mission</button><button class="btn primary" id="gameEndNewGame">Start New Game</button></div></section>`;
      $('#reviewCompletedMission').onclick=()=>showModal(`${mission().number} · ${mission().name}`,`<p><strong>Objective:</strong> ${escapeHtml(mission().objective)}</p><p><strong>Outcome:</strong> ${escapeHtml(missionOutcomeExplanation(state.gameEnd))}</p><div class="wizard-actions"><button class="btn primary" data-close>Done</button></div>`);
  ...
  function strategyActionsStepHtml(d){
    ...
    const soloScuttlingControl=scuttlingSetUp
      ? '<p class="strategy-exception-complete">Macrocyte Warrior set up.</p>'
      : scuttlingResolved
        ? '<p class="strategy-exception-complete">No legal setup location was available.</p>'
        : '<button type="button" class="btn primary big-action" id="scuttlingSetup">Set Up Macrocyte Warrior</button><button type="button" class="btn ghost strategy-exception-action" id="scuttlingNoLegalSetup">No Legal Setup Location</button>';
...
>
  ...
  function bindPlay(){
    ...
    $('#scuttlingSetup')?.addEventListener('click',confirmSoloCeaselessScuttlingSetup);
...
function showPlayerActivation(){
  ...
  function aggressiveDefenseFields(npo,attackType){
    return attackType==='shoot'&&npo?.type==='Canoptek Macrocyte Warrior'
  ...
  function showPendingPlayerAttackWizard(stage,attackType,onResolved,onCancel,preferredTargetId=''){
    ...
    const renderChoices=()=>{
      ...
      $('#aggressiveDefenseFields').innerHTML=aggressiveDefenseFields(target,attackType);
  ...
  function npoSpecialActionDescription(action){
    if(action.id==='geomantic-disturbance')return `<ol><li>Choose a terrain point the Geomancer can see within 8 inches.</li><li>Select every operative within 2 inches of that point.</li><li>The Guide automatically rolls 2D6 separately for each and resolves the damage. If the total is higher than its remaining wounds, deal damage equal to the difference.</li></ol><p>Cannot be used while the Geomancer has a Conceal order or is in enemy control range.</p>`;
    const paragraphs={
      ...
      'nanoscarab-beam':['Choose a visible wounded Canoptek Circle NPO within 6 inches. The Guide automatically rolls 3D3 and restores that many wounds, up to its maximum.','It cannot target an incapacitated NPO or one saved by Reanimate this turning point.']
    };
  ...
  function renderBreachSarcophagusStep(stage){
    ...
    showModal('Breach Sarcophagus',`<div class="summary-box"><strong>${escapeHtml(playerName(context.operativeId))}</strong><br>AP available: ${context.remainingAp}<br>Breach AP cost: ${context.apCost}<br>Destruction Points: ${total} / 20</div><p>Spend ${context.apCost} AP. The Guide automatically rolls 2D6 and adds the total to the sarcophagus’s Destruction Points.</p><div class="wizard-actions breach-navigation"><button class="btn primary" id="performBreach">Perform Breach</button></div><div class="wizard-actions breach-navigation"><button class="btn ghost" id="breachBackToEnemy" aria-label="Back to the enemy control-range question">Back</button></div>`,undefined,'breach-confirmation');
  ...
  function showAbout(){
    showModal('About',`<div class="about-intro">
      ...
      <div class="about-content">
        ...
        <section><h3>Privacy</h3><p>Game state and preferences are stored locally in the user’s browser unless the user explicitly exports or shares them.</p><p><strong>Usage Analytics:</strong> This Guide uses Cloudflare Web Analytics to understand aggregate site traffic such as visits, device type, and referral source. Gameplay state, rosters, dice results, combat results, and saved battles are not intentionally sent to analytics. Offline use is not reported.</p><p class="muted">The app is hosted on GitHub Pages, whose server request logs are governed by GitHub’s own practices.</p></section>
