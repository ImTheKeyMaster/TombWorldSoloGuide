...
>
  ...
  function adjustWounds(id,d){
    ...
    if(wasOut&&d>0&&activeNpos().length>=MAX_NPOS){
      showToast(`Only ${MAX_NPOS} active NPOs can be on the battlefield.`);
      return;
    }
    n.wounds=Math.max(0,Math.min(n.maxWounds,n.wounds+d));
    if(n.wounds===0){n.ready=false;n.deployed=false;n.battlefieldState='out-of-action';}
    else if(wasOut){n.deployed=true;n.battlefieldState='deployed';n.dormant=true;n.ready=false;}
    if(checkGameEnd())return;
    save();render();
  }
