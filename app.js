...
document.addEventListener('touchend',function(e){const now=Date.now();if(now-lastTouchEnd<=300){e.preventDefault();}lastTouchEnd=now;},{passive:false});
  ...
  function setupContent(stepId){
    ...
    if(stepId==='deploy'){
      ...
      return `<h3>Deploy both Kill Teams</h3><p>Place the selected Player operatives in their drop zone and deploy each starting NPO in the mission’s indicated areas.</p><div class="checklist"><label class="check-row"><input id="playerDeployed" type="checkbox" ${state.playerDeployed?'checked':''} ${playerValid?'':'disabled'}><span><strong>${escapeHtml(playerTeamData?.teamName||playerTeamEntry()?.name||'Player')} operatives deployed</strong><small>Confirm that your selected roster has been placed in the Player drop zone.</small></span></label></div>${state.roster.length?`<div class="setup-bulk-row"><button class="btn secondary" id="placeAllNpos" ${allNposPlaced?'disabled':''}>Place All NPOs</button></div>`:''}<div class="deployment-list">${state.roster.length?state.roster.map(n=>`<div class="deployment-item ${n.deployed?'done':''}"><span class="deployment-copy"><strong class="deployment-name">${escapeHtml(npoName(n))}</strong><small class="deployment-type">${escapeHtml(n.behavior)}</small></span><button class="btn ${n.deployed?'ghost':'secondary'} deployment-place-btn" data-deploy="${n.id}">${n.deployed?'Placed':'Mark Placed'}</button></div>`).join(''):'<div class="summary-box"><strong>No starting NPO deployment required.</strong></div>'}</div><div class="wizard-actions"><button class="btn ghost" id="setupBack">Back</button><button class="btn primary" id="setupNext" ${playerValid&&state.playerDeployed&&allNposPlaced?'':'disabled'}>Deployment Complete</button></div>`;
    }
    return `<h3>Setup complete</h3><div class="summary-box"><strong>${mission().number} · ${mission().name}</strong><br>${state.roster.length} starting NPOs<br>${state.playerCount} ${escapeHtml(playerTeamData?.teamName||playerTeamEntry()?.name||'Player')} operatives:<br>${selectedPlayerOperatives().map(o=>escapeHtml(o.name)).join(' · ')}</div>${boardSvg(mission().id)}<p><strong>Mission objective:</strong> ${mission().objective}</p><p><strong>Special rule reminder:</strong> ${missionSpecial()}</p><div class="wizard-actions"><button class="btn ghost" id="setupBack">Back</button><button class="btn primary" id="beginGame">Begin Turning Point 1</button></div>`;
  }
