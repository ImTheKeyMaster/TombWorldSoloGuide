...
An alignment is `VALID` only when it is structurally usable and both hashes equal the current manifest. A malformed document is always `INVALID`; for structurally sound documents, hash comparisons occur before current transcript and duration comparisons so changed source data is reliably reported as `STALE SCRIPT` or `STALE AUDIO`. Stale data is never silently accepted.
