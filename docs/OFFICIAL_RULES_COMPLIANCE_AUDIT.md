...
| COM-02 | P1 | NON-COMPLIANT | Weapon special rules (e.g. Lethal, Piercing/Piercing Crits, Devastating, Brutal, Shock, Stun, Hot, Torrent/Blast) apply exactly as printed. | `playerWeaponProfile()` reads only Attacks, Hit, normal/critical Damage, and an unconditional Piercing/AP value. `weaponPiercingValue()` matches only digits immediately following `Piercing` or `AP`, so `Piercing Crits 1` is ignored rather than applied conditionally. `rollAttack()` treats only an unmodified 6 as critical, so Lethal is ignored; no code applies Devastating or any other listed weapon rule. | `app.js:1535–1557` (`parseWeaponDamage`, `weaponPiercingValue`, `playerWeaponProfile`); `app.js:2257` (`rollAttack`); Player JSON `weapons[].rules` | CORE, **Weapon Rules**; team datacards | Preserve verified base-profile and unconditional Piercing/AP parsing, then implement and test Lethal, Devastating, Piercing Crits and every other automated rule explicitly—or clearly delegate each unsupported rule to tabletop resolution. |
...
* **COM-02 — table-driven unit tests.** **Given** weapons with unconditional Piercing/AP, Piercing Crits, Lethal, Devastating, Brutal, Shock, Stun, Hot, Torrent and Blast plus critical/noncritical roll patterns, **when** profiles and combat resolve, **then** unconditional AP reduces defence correctly, conditional Piercing triggers only from a qualifying critical, Lethal changes the critical threshold, Devastating inflicts its critical damage, and every other supported rule produces its exact official effect once; unsupported rules must be identified and delegated rather than silently ignored.
...
* **Scope:** separate Shoot/Fight engines; correct cover/AP dice pool; distinct NPO weapons; explicit handling for Lethal, Devastating, Piercing Crits and every other automated special rule; durable combat transaction.
...
| Attack profiles/rules | `app.js`, player JSON | `profiles[].attack`, player `weapons`, `parseWeaponDamage`, `weaponPiercingValue`, `playerWeaponProfile`, `rollAttack` | Player parsing consumes base characteristics plus unconditional Piercing/AP only; NPOs have one profile; all other special-rule handling is absent. |
