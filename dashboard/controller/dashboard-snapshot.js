...
export function createDashboardSnapshot(source, revision, updatedAt = Date.now()) {
  const snapshot = {
    ...
    mission: { id: text(source?.mission?.id), number: text(source?.mission?.number), name: text(source?.mission?.name), summary: text(source?.mission?.summary), progress: number(source?.mission?.progress), target: number(source?.mission?.target), objectives: list(source?.mission?.objectives) },
    currentActivation: source?.currentActivation ? { ...source.currentActivation } : null,
    ...
    activeEvents: list(source?.activeEvents),
