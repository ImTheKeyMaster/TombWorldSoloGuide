...
async function pair() {
  ...
  try {
    ...
    await peer.setRemoteDescription({ type: offer.sdpType, sdp: offer.sdp }); await addRemoteIceCandidates(peer, offer.candidates, offer.sdp); await peer.setLocalDescription(await peer.createAnswer()); const gathered = await collectIceCandidates(peer, { maximumWaitMs: DASHBOARD_CONFIG.connectionTimeoutMs, quietPeriodMs: DASHBOARD_CONFIG.iceCandidateQuietPeriodMs }); const answerCreatedAt = Date.now(); if (answerCreatedAt >= offer.expiresAt) throw new Error('Pairing payload has expired.');
