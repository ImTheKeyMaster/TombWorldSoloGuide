...
import { collectIceCandidates, addRemoteIceCandidates } from './shared/webrtc-ice.js';
...
async function pair() {
  ...
  try {
    ...
    await peer.setRemoteDescription({ type: offer.sdpType, sdp: offer.sdp }); await addRemoteIceCandidates(peer, offer.candidates, offer.sdp); const gathered = await collectIceCandidates(peer, { maximumWaitMs: DASHBOARD_CONFIG.connectionTimeoutMs, quietPeriodMs: DASHBOARD_CONFIG.iceCandidateQuietPeriodMs, startGathering: async () => peer.setLocalDescription(await peer.createAnswer()) }); const answerCreatedAt = Date.now(); if (answerCreatedAt >= offer.expiresAt) throw new Error('Pairing payload has expired.');
    const answer = await serializePairingData({ protocolVersion: DASHBOARD_PROTOCOL_VERSION, type: 'answer', nonce: offer.nonce, createdAt: answerCreatedAt, expiresAt: offer.expiresAt, sdpType: 'answer', sdp: peer.localDescription.sdp, candidates: gathered.candidates, label: offer.label }); responseOutput.value = answer; await renderQrCode(document.querySelector('[data-response-qr]'), answer, 'Scan this response QR code with the game device.'); pairing.hidden = false; const sessionCode = await codeFor(offer.nonce); verification.querySelector('strong').textContent = sessionCode; setText('[data-session-id]', sessionCode); document.querySelector('[data-copy-response]').onclick = async () => { await navigator.clipboard.writeText(answer); setStatus('Response copied: return to game device', 'connecting'); }; setStatus('Nexus link: awaiting response scan', 'connecting');
