...
function renderEvents(events) {
  ...
  events.forEach(event => { const item = document.createElement('li'); item.className = event.category === 'critical' || event.severity === 'critical' ? 'critical' : 'warning'; addTextElement(item, 'b', `⚠ ${event.title || 'Active event'}`); addTextElement(item, 'span', event.summary || 'No event description received.'); if (event.effect) addTextElement(item, 'span', `Effect: ${event.effect}`); container.append(item); });
}
...
function renderNarrative(entries) {
  ...
  if (!entries.length) { const item = document.createElement('li'); addTextElement(item, 'time', 'SYS'); addTextElement(item, 'span', 'Tactical relay stable. No narrative messages received.'); container.append(item); return; }
...
async function pair() {
  ...
  try {
    ...
    peer.ondatachannel = event => { const channel = event.channel; if (channel.label !== 'tomb-world-dashboard') { channel.close(); return; } const send = type => channel.send(JSON.stringify({ protocolVersion: DASHBOARD_PROTOCOL_VERSION, type })); channel.onopen = () => send(DASHBOARD_MESSAGE_TYPES.DASHBOARD_READY); channel.onmessage = messageEvent => { const message = safeParseDashboardJson(messageEvent.data); if (!validateDashboardMessage(message).valid) return; if (message.type === DASHBOARD_MESSAGE_TYPES.HELLO) { send(DASHBOARD_MESSAGE_TYPES.HELLO_ACK); send(DASHBOARD_MESSAGE_TYPES.REQUEST_SNAPSHOT); connectedAt = Date.now(); setStatus('Nexus link: active', 'connected'); setText('[data-link-quality]', 'Optimal'); pairing.hidden = true; verification.hidden = false; } else if (message.type === DASHBOARD_MESSAGE_TYPES.SNAPSHOT) acceptDashboardSnapshot(message.snapshot); else if (message.type === DASHBOARD_MESSAGE_TYPES.PING) send(DASHBOARD_MESSAGE_TYPES.PONG); }; channel.onclose = () => { setStatus('Nexus link: connection interrupted', 'interrupted'); setText('[data-link-quality]', 'Interrupted'); }; };
